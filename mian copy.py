...
if __name__ == '__main__':
    # 读取文本文件
    file_path = '三国演义.txt'  # 替换为你的文件路径
    with open(file_path, 'r', encoding='utf-8') as file:
        content = file.read()  # 读取整个文件内容

    # 按回车和句号分割内容
    split_content = [sentence.strip() for sentence in content.replace('。', '\n').replace('；', '\n').replace('！', '\n').replace('？', '\n').split('\n') if sentence.strip()]

    # 将分割后的内容转换为 DataFrame
    df = pd.DataFrame(split_content, columns=['Text'])
    ...
    filePath2 = 'edge.csv'
    # 读取csv文件获取数据并存储到列表中
    df_ = [w for w in df['Text'] if len(w)>1]
